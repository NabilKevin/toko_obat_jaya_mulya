...
const openProfitModal = async (e) => {
    ...
    try {
        ...
        const targetUrl = `${window.BASE_URL}admin/transaksi/profit?${params.toString()}`;
        ...
        document.getElementById('profitTotalJual').innerText = 'Rp ' + formatNumber(data.total_jual);
        document.getElementById('profitTotalModal').innerText = 'Rp ' + formatNumber(data.total_modal);
        document.getElementById('profitTotalUntung').innerText = 'Rp ' + formatNumber(data.keuntungan);
...
function openReturnModal(trxId, kode) {
    ...
    document.getElementById('returnForm').action =
        `${BASE_URL}admin/transaksi/${trxId}/return`;
...
function loadReturnItems(trxId) {
    ...
    document.getElementById('return-items').innerHTML =
        html || `<tr><td colspan="4" class="text-center py-4 text-muted-foreground">
...
window.openVoidModal = function(id, kode, event) {
    ...
    document.getElementById('voidForm').action = `${window.BASE_URL}admin/transaksi/${id}/void`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Tombol Close Detail
    document.getElementById('trx-close-bottom')?.addEventListener('click', closeAllModals);
    
    // Klik Overlay untuk menutup
    document.querySelectorAll('[data-modal-overlay], [data-profit-overlay]').forEach(el => {
...
function printStruk() {
    ...
    window.open(
        `${window.BASE_URL}admin/struk/${activeTransaction.kode}`,
        '_blank'
    );
}
// Print Struk
document.getElementById('trx-print')?.addEventListener('click', () => {
    ...
    if (kode) window.open(`${window.BASE_URL}admin/struk/${kode}`, '_blank');
});