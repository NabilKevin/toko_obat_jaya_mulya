        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm p-2">
                <thead class="bg-neutral-50 dark:bg-neutral-900">
                    <tr>
                        <th
                            class="text-center py-3 px-2 text-xs sm:text-sm font-semibold text-muted-foreground bg-slate-100/50 dark:bg-slate-800/50 rounded-l-lg">
                            Kode</th>
                        <th
                            class="text-center py-3 px-2 text-xs sm:text-sm font-semibold text-muted-foreground bg-slate-100/50 dark:bg-slate-800/50 hidden sm:table-cell">
                            Obat</th>
                        <th
                            class="text-center py-3 px-2 text-xs sm:text-sm font-semibold text-muted-foreground bg-slate-100/50 dark:bg-slate-800/50 hidden md:table-cell">
                            Jumlah</th>
                        <th
                            class="text-center py-3 px-2 text-xs sm:text-sm font-semibold text-muted-foreground bg-slate-100/50 dark:bg-slate-800/50">
                            Total</th>
                        <th
                            class="text-center py-3 px-2 text-xs sm:text-sm font-semibold text-muted-foreground bg-slate-100/50 dark:bg-slate-800/50 rounded-r-lg hidden md:table-cell">
                            Waktu</th>
                        <th
                            class="text-center py-3 px-2 text-xs sm:text-sm font-semibold text-muted-foreground bg-slate-100/50 dark:bg-slate-800/50 rounded-r-lg">
                            Aksi</th>
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr class="border-b dark:border-neutral-800">
                            ...
                            <td class="text-center px-4 py-2">
                                <button data-id="{{ $transaksi->id }}"
                                    class="px-3 py-1.5 rounded-md border border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800 btn-detail">Detail</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            ...
            <div class="px-4 md:px-6 py-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
                <div
                    ...
                    <div class="text-sm text-gray-700 dark:text-gray-300 text-center md:text-left">
                        Menampilkan <span class="font-medium">{{ $transaksis->firstItem() }}</span> sampai <span
                            class="font-medium">{{ $transaksis->lastItem() }}</span> dari <span
                            class="font-medium">{{ $transaksis->total() }}</span> hasil
                    </div>
                        ...
                        <div class="flex gap-4 items-center flex-col md:flex-row">
                            <div class="flex items-center space-x-1 sm:space-x-2">
                                ...
                                <button
                                    ...
                                    @disabled($transaksis->onFirstPage())
                                    onclick="window.location.href='{{ $transaksis->url(1) }}'">
                                ...
                                <button
                                    class="group px-2 sm:px-3 py-2.5 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200 flex items-center font-medium min-h-[44px] sm:min-h-[48px] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                                    @disabled($transaksis->onFirstPage())
                                    onclick="window.location.href='{{ $transaksis->previousPageUrl() }}'">
                                ...
                                <div class="flex items-center space-x-1">
                                    <!-- Current page -->
                                    <button
                                        class="px-3 py-2.5 text-sm bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg font-medium shadow-md min-h-[48px] ring-2 ring-blue-200 dark:ring-blue-800">{{ $transaksis->currentPage() }}</button>
                                </div>

                                <!-- Next Page -->
                                <button
                                    class="group px-2 sm:px-3 py-2.5 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200 flex items-center font-medium min-h-[44px] sm:min-h-[48px] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                                    @disabled($transaksis->onLastPage())
                                    onclick="window.location.href='{{ $transaksis->nextPageUrl() }}'">
                                ...
                                <button
                                    class="group px-2 sm:px-3 py-2.5 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200 flex items-center font-medium min-h-[44px] sm:min-h-[48px] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                                    @disabled($transaksis->onLastPage())
                                    onclick="window.location.href='{{ $transaksis->url($transaksis->lastPage()) }}'">
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="trx-modal-title"
            ...
            <div class="flex items-center justify-between px-4 py-3 border-b border-neutral-200 dark:border-neutral-800">
                <div>
                    ...
                    <p id="trx-meta" class="text-xs text-muted-foreground">Kode transaksi: <span id="trxKode"></span>
                    </p>
            ...
            <div class="px-4 pb-4">
                <div class="flex items-center gap-2">
                    <button id="trx-close-bottom"
                        class="h-10 px-4 rounded-md bg-gradient-to-r from-blue-600 to-blue-700 text-white active:scale-90 transition-all">Selesai</button>
                    <button id="trx-print"
                        class="h-10 px-4 rounded-md bg-gradient-to-r from-green-600 to-green-700 text-white active:scale-90 transition-all">Print</button>
                </div>
            </div>
        </div>
    </div>
